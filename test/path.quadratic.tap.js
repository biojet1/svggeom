...
// import { Quadratic } from '../dist/path/segment/pathse.js';
...
for await (const item of enum_path_data({ SEGMENTS: 'QuadraticBezier' })) {
    ...
    test.test(`SegmentLS<${item.d}>`, { bail: CI }, function (t) {
        ...
        {
            const cur = SegmentLS.move_to(start).quad_to(p, end);
        ...
        // if (CI)
        {
            testSegment(t, SegmentLS.parse(`M ${sx},${sy} Q ${x1},${y1} ${ex},${ey}`), item, deltp);
            testSegment(t, SegmentLS.parse(`m ${sx},${sy} q ${x1 - sx},${y1 - sy} ${ex - sx},${ey - sy}`), item, deltp);
        }
        t.end();
    });
}
