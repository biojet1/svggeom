...
test.Test.prototype.addAssert(
	'sameBox',
	3,
	function (box1, box2, epsilon = 0.0001, message, extra) {
		if (box1.bbox) {
		...
		for (const [i, k] of [`left`, `right`, `top`, `bottom`]) {
			if (!almostEqual(box1[i], box2[i], epsilon)) {
				this.fail(
					`Box ${k} ${box1[i]} not close to ${box2[i]} ${
						box1[i] - box2[i]
					}Δ ±${epsilon} : ${message}`
				);
				// this.end();
...
test.Test.prototype.addAssert(
	'almostEqual',
	3,
	function (A, B, opt, message, extra) {
		...
		for (; i < n; ++i) {
			...
			if (v < 1) {
				opt?.on_fail?.();
				this.fail(
					`<${a}> not close to <${b}> ${a - b}Δ @${i} ±${epsilon} ${message}`,
					extra
				);
				return;
			}
		...
		if (i > 1) {
			...
			this.fail(
				` nothing to compare close to ${A} ${B} ±${epsilon} ${message}`,
				extra
			);
		}
	}
...
function dbgwrite(dbg, pC, pX) {
	function* gen() {
		...
		if (dbg.path_source) {
			style =
				'fill:yellow;stroke:orange;stroke-width:4;stroke-dasharray:none;opacity:0.6';

			yield `<path id="source" d="${dbg.path_source}" style="${style}"/>`;
			style =
				'fill:skyblue;stroke:blue;stroke-width:4;stroke-dasharray:none;opacity:0.6';
		...
		style =
			'fill:firebrick;stroke:red;stroke-width:2;stroke-dasharray:none;opacity:0.6';

		yield `<path id="calculated" d="${pC.join(' ')}" style="${style}"/>`;
		style =
			'fill:palegreen;stroke:green;stroke-width:1;stroke-dasharray:none;opacity:0.6';
...
test.Test.prototype.addAssert(
	'sameDescs',
	3,
	function (a, b, opt, message, extra = {}, debug_svg = null) {
		opt = typeof opt == 'number' ? {epsilon: opt} : opt;
		const epsilon = opt?.epsilon || 1e-13;
		...
		for (let i = 0; i < n; i++) {
			...
			if (A == B) {
				...
				if (d <= epsilon) {
					...
					return this.fail(
						`item #${i} ${d}Δ [${A}, ${B}] ±${epsilon} "${message}"`,
						extra
					);
				}
			}
...
test.Test.prototype.addAssert(
	'sameTangent',
	3,
	function (a, b, epsilon, message, extra = {}) {
		...
		if (a[0] === b[0]) {
			if (a[1] === b[1]) {
				...
				return this.fail(
					`Tangent @y ${a[1]} not close to ${b[1]} ${
						a[1] - b[1]
					}Δ ±${delta_epsilon} : ${message}`,
					extra
				);
			}
		} else if (a[1] === b[1]) {
			if (a[1] == 0) {
				...
				return this.fail(
					`Tangent @x ${a[0]} not close to ${b[0]} ${
						a[0] - b[0]
					}Δ ±${delta_epsilon} : ${message}`,
					extra
				);
			...
			if (!almostEqual(A, B, slope_epsilon)) {
				return this.fail(
					`Tangent / ${A} not close to ${B} ${
						A - B
					}Δ ±${slope_epsilon}  [${a}], [${b}], Δ[${a[0] - b[0]}, ${
						a[1] - b[1]
					}] : ${message}`,
					extra
				);
