...
import {Cubic, Path} from '../dist/path.js';
	...
	for (const [T, {x, y, tx, ty, pathA, pathB}] of Object.entries(item.at)) {
		...
		if (test_descs && pathA) {
			try {
				...
				t.sameDescs(descArray(a), pathA, point_epsilon, `splitAt(0, ${T})`, [
					item,
					seg,
				]);
				t.sameDescs(
